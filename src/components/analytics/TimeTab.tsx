...
function BubbleTip({ active, payload }: { active?: boolean; payload?: readonly { payload: { hour: string; pnl: number; z: number } }[] }) {
  if (!active || !payload?.[0]) return null;
  const { hour, pnl, z } = payload[0].payload;
  return (
    <div className="bg-card border border-border rounded-lg px-3 py-2 text-xs shadow-lg space-y-0.5">
      <p className="font-bold text-foreground">{hour} ET</p>
      <p className={pnl >= 0 ? "text-profit" : "text-loss"}>{fmt.dollars(pnl)}</p>
      <p className="text-muted-foreground">|{fmt.dollars(z)}|</p>
    </div>
  );
}
...
export function TimeTab({ analytics: a }: TimeTabProps) {
  const bubbleWinners = a.bubble_scatter.filter((d) => d.pnl >= 0);
  const bubbleLosers  = a.bubble_scatter.filter((d) => d.pnl < 0);

  const durWinners = a.duration_scatter.filter((d) => d.pnl >= 0);
  const durLosers  = a.duration_scatter.filter((d) => d.pnl < 0);
